...
for batch_title in "$@"; do
	...
	mkdir unopt_pdbs pm7 rm1-d sp-dft sp-tddft t1_sp-tddft s0_vac s0_solv s1_solv t1_solv cat-rad_solv cat-rad_vac all-logs mol-data flow-tools
	for d in pm7 rm1-d sp-dft sp-tddft t1_sp-tddft; do mkdir $d/completed $d/failed; done
