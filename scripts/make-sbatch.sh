...
# default commands
if [ -z "$COMMANDS" ]; then
	if [[ "$TYPE" == "com" ]]; then COMMANDS=$templates/commands/default_g16.txt
	elif [[ "$TYPE" == "inp" ]]; then COMMANDS=$templates/commands/default_gamess.txt
...
# add commands
cat $COMMANDS | sed "s/JOBNAME/$JOBNAME/g" >> $SBATCH
