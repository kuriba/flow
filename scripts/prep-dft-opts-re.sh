...
bash $FLOW/scripts/make-com.sh -i=$pdb_file -r='#p M06/6-31+G(d,p) SCRF=(Solvent=Acetonitrile) opt' -t=$inchi\_S0_solv -c=$charge -l="$S0_SOLV/resubmits"
...
if [ ! -z $n ]; then
	...
	bash $FLOW/scripts/make-com.sh -i=$pdb_file -r="#p M06/6-31+G(d,p) SCRF=(Solvent=Acetonitrile) opt td=root=$n" -t=$inchi\_S$n\_solv -c=$charge -l=$SN_SOLV
fi

bash $FLOW/scripts/make-com.sh -i=$pdb_file -r='#p M06/6-31+G(d,p) SCRF=(Solvent=Acetonitrile) opt td=root=1' -t=$inchi\_S1_solv -c=$charge -l="$S1_SOLV/resubmits"
...
bash $FLOW/scripts/make-com.sh -i=$pdb_file -r='#p M06/6-31+G(d,p) SCRF=(Solvent=Acetonitrile) opt' -t=$inchi\_T1_solv -c=$charge -s=3 -l="$T1_SOLV/resubmits"
...
bash $FLOW/scripts/make-com.sh -i=$pdb_file -r='#p M06/6-31+G(d,p) SCRF=(Solvent=Acetonitrile) opt' -t=$inchi\_cat-rad_solv -c=$(($charge + 1)) -s=2 -l="$CAT_RAD_SOLV/resubmits"
...
bash $FLOW/scripts/make-com.sh -i=$pdb_file -r='#p M06/6-31+G(d,p) opt' -t=$inchi\_cat-rad_vac -c=$(($charge + 1)) -s=2 -l="$CAT_RAD_VAC/resubmits"
