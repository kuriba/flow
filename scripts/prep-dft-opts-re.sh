...
cd "$S0_SOLV/resubmits" && setup-sbatch "$inchi\_S0_solv.com" "$FLOW_TOOLS/templates/single_g16_dft-opt.sbatch"
...
cd "$S1_SOLV/resubmits" && setup-sbatch "$inchi\_S1_solv.com" "$FLOW_TOOLS/templates/single_g16_dft-opt.sbatch"

# setup T1 DFT optimization (in solvent)
bash $FLOW_TOOLS/scripts/make-com.sh -i=$pdb_file -r='#p M06/6-31+G(d,p) SCRF=(Solvent=Acetonitrile) opt' -t=$inchi\_T1_solv -s=3 -l="$T1_SOLV/resubmits"
cd "$T1_SOLV/resubmits" && setup-sbatch "$inchi\_T1_solv.com" "$FLOW_TOOLS/templates/single_g16_dft-opt.sbatch"
...
cd "$CAT_RAD_SOLV/resubmits" && setup-sbatch "$inchi\_cat-rad_solv.com" "$FLOW_TOOLS/templates/single_g16_dft-opt.sbatch"
...
cd "$CAT_RAD_VAC/resubmits" && setup-sbatch "$inchi\_cat-rad_vac.com" "$FLOW_TOOLS/templates/single_g16_dft-opt.sbatch"
